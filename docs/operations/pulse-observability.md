...
Stage 16.4 adopts first-party Laravel Pulse for technical observability. `/admin` remains a business operations surface; `/pulse` is the engineering/operations surface and is authorized by the `viewPulse` Gate through `User::viewPulse Laravel Gate capability`.

## Local / Laragon

Pulse uses the primary PostgreSQL connection unless `PULSE_DB_CONNECTION` names a dedicated connection. Run `php artisan migrate` after installation, then visit `/pulse` as an active `system_operator` or `super_admin`.

The default ingest driver is `storage`, which avoids a second Redis stream worker during local development.

## Production profile

For high traffic, use a dedicated Pulse database connection and optionally Redis ingest. If `PULSE_INGEST_DRIVER=redis`, `PULSE_REDIS_CONNECTION` must point to a Redis connection separate from the Redis queue connection. Run `php artisan pulse:work` under a process monitor and `php artisan pulse:check` on each application server.
