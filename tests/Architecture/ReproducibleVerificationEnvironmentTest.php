...
it('defines a canonical Docker verification environment with PostgreSQL 18', function (): void {
    ...
    expect($compose)
        ...
        ->toContain('songchart/verify-runtime:local')
        ->toContain('condition: service_healthy')
        ->and($dockerfile)
        ->toContain('FROM php:8.5-cli-bookworm@sha256:')
        ->toContain('FROM node:24-bookworm-slim@sha256:')
        ->toContain('FROM composer:2@sha256:')
        ->toContain('pdo_pgsql')
