...
final class DemoSearchCatalog implements SearchCatalog
{
    ...
    private array $items = [
        ['type' => 'artist', 'artist_type' => 'group', 'label' => 'Nhóm nhạc', 'slug' => 'radiohead', 'title' => 'Radiohead', 'context' => 'Ban nhạc alternative rock · Anh', 'meta' => 'Thành lập 1985 · Canonical identity', 'year' => 1985, 'verified' => true, 'description' => 'Ban nhạc rock Anh được biết đến với cách tiếp cận thử nghiệm và nhiều giai đoạn âm thanh khác nhau.'],
        ...
        ['type' => 'artist', 'artist_type' => 'group', 'label' => 'Nhóm nhạc', 'slug' => 'nirvana', 'title' => 'Nirvana', 'context' => 'Ban nhạc grunge · Hoa Kỳ', 'meta' => 'Thành lập 1987 · Canonical identity', 'year' => 1987, 'verified' => true, 'description' => 'Ban nhạc rock Mỹ có ảnh hưởng lớn đến grunge.'],
    ...
    public function search(string $query, string $type = 'all', string $sort = 'relevance', int $page = 1): array
    {
        ...
        $pageItems = array_map(fn (array $item): array => array_merge($item, ['url' => PublicEntityUrl::to((string) $item['type'], (string) $item['slug'], isset($item['artist_type']) ? (string) $item['artist_type'] : null)]), array_slice($items, $offset, $perPage));
    ...
    public function find(string $type, string $slug): ?array
    {
        foreach ($this->items as $item) {
            ...
            return array_merge($item, ['url' => PublicEntityUrl::to((string) $item['type'], (string) $item['slug'], isset($item['artist_type']) ? (string) $item['artist_type'] : null)], $this->detailPayload($item));
        }
    ...
    private function detailPayload(array $item): array
    {
        ...
        return array_merge($details, [
            'passport' => $this->demoPassport($item, $details),
            'providers' => [
                [
    ...
    /**
     * Keep the testing/local fallback payload aligned with EntityPassportReadModel.
     *
     * @param  array<string, mixed>  $item
     * @param  array<string, mixed>  $details
     * @return array<string, mixed>
     */
    private function demoPassport(array $item, array $details): array
    {
        $identifiers = array_map(
            static fn (array $identifier): array => [
                'namespace' => (string) $identifier['scheme'],
        ...
        $needs = $identifiers === [] ? [[
            ...
            'priority' => 'normal',
            'cost_class' => 'low',
            'reason' => 'Demo fixture has no verified external identifier for this entity.',
        ...
        return [
            ...
            'identity_bridge' => [
                ...
                'destinations' => [],
                'connected_source_count' => count($identifiers),
            ],
            'enrichment_plan' => [
                'completeness' => $needs === [] ? 100 : 75,
                'needs' => $needs,
            ],
        ];
    }
        ...
        foreach ($keys as [$type, $slug]) {
            foreach ($this->items as $candidate) {
                if ($candidate['type'] === $type && $candidate['slug'] === $slug) {
                    $related[] = array_merge($candidate, ['url' => PublicEntityUrl::to((string) $candidate['type'], (string) $candidate['slug'], isset($candidate['artist_type']) ? (string) $candidate['artist_type'] : null)]);
                    break;
                }
            }
        }
