...
final class PublicCatalogReadModel
{
    ...
    public function index(EntityType $type, string $query = '', ?string $artistScope = null): array
    {
        ...
        if ($type === EntityType::Artist && $artistScope === 'solo') {
            $builder->where('artist_type', 'person');
        }
        if ($type === EntityType::Artist && $artistScope === 'group') {
            $builder->whereIn('artist_type', ['group', 'orchestra', 'choir']);
        }
        ...
        return [
            ...
            'title' => [
                EntityType::Artist->value => $artistScope === 'group' ? 'Nhóm nhạc' : 'Nghệ sĩ',
            ...
            'description' => [
                EntityType::Artist->value => $artistScope === 'group' ? 'Khám phá nhóm nhạc, dàn nhạc và hợp xướng canonical của SongChart.' : 'Khám phá nghệ sĩ solo canonical của SongChart.',
