...
PY2
mkdir -p bootstrap/cache storage/framework/cache/data storage/framework/sessions storage/framework/views storage/logs .certs
if [[ ! -f ".certs/$DOMAIN.pem" || ! -f ".certs/$DOMAIN-key.pem" ]]; then
 command -v mkcert >/dev/null || { echo 'mkcert is required for HTTPS. Install it in Ubuntu, then rerun ./songchart dev setup' >&2; exit 1; }
 mkcert -install
