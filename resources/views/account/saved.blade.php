    ...
    <div class="space-y-3">
            ...
            <x-ui.card class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    @else
                        ...
                        <p class="mt-1 text-sm text-[var(--sc-text-muted)]">Bản ghi đã lưu được giữ an toàn nhưng không còn resolve được tới public canonical entity.</p>
                    @endif
                </div>
        ...
        @empty
            <x-ui.card>
                ...
                <p class="mt-2 text-sm text-[var(--sc-text-secondary)]">Mở một trang nghệ sĩ, bản thu, phát hành hoặc thực thể canonical khác để lưu lại.</p>
            </x-ui.card>
        @endforelse
    </div>
</x-account.shell>
