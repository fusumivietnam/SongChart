...
<section class="sc-home-section">
    <div class="sc-container">
        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
            ...
            <aside class="rounded-[var(--sc-radius-panel)] border border-[var(--sc-border)] bg-[var(--sc-bg-surface)] p-5" aria-label="Trạng thái dữ liệu bảng xếp hạng">
                ...
                @else
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $chart['freshness'] === 'fresh' ? 'bg-[var(--sc-success-soft)] text-[var(--sc-success)]' : ($chart['freshness'] === 'stale' ? 'bg-[var(--sc-warning-soft)] text-[var(--sc-warning)]' : 'bg-[var(--sc-bg-subtle)] text-[var(--sc-text-secondary)]') }}">
        ...
        @if($chart['state'] === 'unavailable')
            <div class="mt-8 rounded-[var(--sc-radius-panel)] border border-dashed border-[var(--sc-border)] bg-[var(--sc-bg-surface)] p-8 text-center">
        ...
        @elseif(count($chart['rows']) === 0)
            <div class="mt-8 rounded-[var(--sc-radius-panel)] border border-[var(--sc-border)] bg-[var(--sc-bg-surface)] p-8 text-center">
        ...
        @else
            <ol class="mt-8 grid gap-4" aria-label="Các vị trí trong bảng xếp hạng">
                @foreach($chart['rows'] as $row)
                    <li class="rounded-[var(--sc-radius-panel)] border border-[var(--sc-border)] bg-[var(--sc-bg-surface)] p-5 md:p-6">
                        ...
                        <details class="mt-4 border-t border-[var(--sc-border)] pt-4">
                            ...
                            <div class="mt-3 grid gap-3">
                                @forelse($row['observations'] as $observation)
                                    <div class="rounded-xl bg-[var(--sc-bg-subtle)] p-4 text-sm">
