        ...
        <x-ui.card class="mt-8">
            <div class="p-6">
                ...
                <p class="mt-2 text-sm text-[var(--sc-text-secondary)]">
                    ...
                    @elseif($entityType === 'artist' && ($artistScope ?? null) === 'group')
                        Chưa có nhóm nhạc canonical phù hợp. Hãy import Group Artist từ MusicBrainz rồi quay lại trang này.
                    @else
                        Chưa có nghệ sĩ solo canonical phù hợp. Hãy import Person Artist từ MusicBrainz rồi quay lại trang này.
                    @endif
                </p>
            </div>
        </x-ui.card>
    @else
        <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($entities as $entity)
                @php($showUrl = \App\Support\Catalog\PublicEntityUrl::to($entityType, (string) $entity->slug, $entityType === 'artist' ? (string) $entity->artist_type : null))
