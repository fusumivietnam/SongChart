...
<div data-admin-dashboard="attention-first">
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-2">
        <section aria-labelledby="provider-heading"><x-ui.card><div class="p-5"><div class="flex items-start justify-between"><div><h2 id="provider-heading" class="text-xl font-bold">Nguồn dữ liệu</h2><p class="mt-1 text-sm text-slate-500">Tình trạng các nguồn đang cung cấp metadata cho hệ thống.</p></div><a class="text-sm font-semibold underline" href="{{ route('admin.providers.index') }}">Xem tất cả</a></div><div class="mt-5 space-y-3">@forelse($providers->take(6) as $provider)<div class="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><span class="h-2.5 w-2.5 rounded-full {{ $provider->is_enabled ? 'bg-emerald-500' : 'bg-amber-500' }}"></span><div class="min-w-0 flex-1"><p class="truncate text-sm font-semibold">{{ $provider->name }}</p><p class="text-xs text-slate-500">{{ $provider->is_enabled ? 'Đang sử dụng' : 'Đang tạm ngừng' }}</p></div></div>@empty<x-ui.empty-state title="Chưa có nguồn dữ liệu" description="Nguồn dữ liệu sẽ xuất hiện ở đây sau khi được đăng ký." />@endforelse</div></div></x-ui.card></section>
