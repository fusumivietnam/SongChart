...
<div class="min-w-0"><p class="text-sm font-semibold text-[var(--sc-primary)]">{{ $entity['eyebrow'] }}</p><div class="mt-2 flex flex-wrap gap-2"><x-ui.badge :variant="$entity['type']">{{ $entity['label'] }}</x-ui.badge>@if($entity['verified'])<x-ui.badge variant="success">Canonical đã xác minh</x-ui.badge>@else<x-ui.badge variant="warning">Một số thông tin chưa được xác minh</x-ui.badge>@endif</div><h1 id="entity-title" class="mt-3 break-words text-4xl font-bold tracking-tight">{{ $entity['title'] }}</h1><p class="mt-2 text-lg text-[var(--sc-text-secondary)]">{{ $entity['context'] }}</p><p class="mt-3 text-sm text-[var(--sc-text-muted)]">{{ $entity['meta'] }}</p></div>
</header>
...
<x-ui.card><div class="p-4"><p class="text-sm font-semibold">External identities</p><ul class="mt-3 space-y-2 break-words text-sm text-[var(--sc-text-secondary)]">@forelse($entity['passport']['identity_bridge']['identifiers'] as $identity)<li><strong>{{ $identity['namespace'] }}</strong>: {{ $identity['value'] }}</li>@empty<li>No external identity has been verified yet.</li>@endforelse</ul></div></x-ui.card>
<x-ui.card><div class="p-4"><p class="text-sm font-semibold">Provider destinations</p><ul class="mt-3 space-y-2 break-words text-sm text-[var(--sc-text-secondary)]">@forelse($entity['passport']['identity_bridge']['destinations'] as $destination)<li><strong>{{ $destination['provider'] }}</strong>: {{ $destination['review_state'] }}</li>@empty<li>No provider destination is linked yet.</li>@endforelse</ul></div></x-ui.card>
</div>
</section>
